...
В основе лежит публичный датасет [Severstal Steel Defect Detection](https://www.kaggle.com/competitions/severstal-steel-defect-detection), однако он был видоизменен для приближения к реальным производственным условиям. Данные могут содержать шум в разметке — часть вашего задания изученными технологиями минимизировать его.
...
- Проведите error analysis: на каких классах модель ошибается, какие типы ошибок преобладают (для модели YOLO вы сможете изучить эти метрики как в конечном логе обучения, так и в results.csv внутри папки вашего эксперимента).
...
- Обратите внимание что данная задача не имеет только одного итогового верного решения, основная цель - добиться максимально высоких метрик у модели, которая пройдет по критерию FPS.
